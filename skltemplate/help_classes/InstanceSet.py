...
class InstanceSet:
   ...
   # Attribute where all the instances of the DB are stored.
    ...
    data_folder = cwd + "\\biodata\\dataset\\"
    file_to_open= None
    data_lines=None
    ...
    def insertAttribute(self,line):
        ...
        print("The length of st is :" + str(len(st)))

        for item_string in st:
            print(item_string+'\n')
        ...
        if (len(st)==1):  # Parsing a nominal attribute with no definition of values
            ...
            attType = st[2].lower()
            print("attribute Name : " + str(st[1]) + ", attribute type = "+ str(attType))
